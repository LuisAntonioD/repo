...
if (document.getElementById('navbarBlur')) {
  Dashboard.navbarBlurOnScroll('navbarBlur');
}
...
window.addEventListener('resize', function(event) {
  ...
  total.forEach(function(item, i) {
    if(item && item.querySelector('.moving-tab') && item.querySelector(".nav-link")){
...
var Dashboard = {
  ...
  initNavs: (container = null) => {
    if(container)
      var total = container.querySelectorAll('.nav-pills')
    else
      var total = document.querySelectorAll('.nav-pills')
      ...
      total.forEach(function(item, i) {
        ...
        if(first_li){
          ...
          tab.classList.add('h-100')
          tab.innerHTML = "&nbsp;";
      
          moving_div.classList.add('moving-tab', 'position-absolute', 'nav-link', 'h-100');
          moving_div.appendChild(tab);
          item.appendChild(moving_div);
          ...
          item.onmouseover = function(event) {
            let target = Dashboard.getEventTarget(event);
    ...
    navbarBlurOnScroll: (id) => {
      ...
      if (navbarScrollActive == 'true') {
        window.onscroll = Dashboard.debounce(function() {
      ...
      if (isWindows) {
        ...
        if (navbarScrollActive == 'true') {
          content.addEventListener('ps-scroll-y', Dashboard.debounce(function() {
