$(function(){
    ...
    $(this).on('click', '#consultar', function(){
        ...
        let id_ticket = $(this).data('id-ticket'); 
         $('#myModalTicketCancelado').modal('show');
         $('#myModalTicketCancelado .modal-header .modal-title').html(`<strong>Historial del Ticket <strong id="id_ticket">${id_ticket}</strong></strong>`);
         $('#myModalTicketCancelado .modal-footer').html(`<button type="button" class="btn btn-default" data-bs-dismiss="modal">Cerrar</button>`);
         TicketsCancelados.historialTicket(id_ticket)
     });

     //BOTON FILTRAR
    $("#btn_aplicar_filtros").on('click',function(e){
        TicketsCancelados.llenaTabla()
	...
	TicketsCancelados.tablaTicketsCancelados = $("#tablaTicketsCancelados").DataTable({
		responsive: true,
		paging: true,
		searching: true,
		ordering: true,
            ...
            {data: "fecha", className: "text-center"},
            {
				...
				render: function (data, type, row) {
					return `<div class="action-buttons btn-group">
                                <span id="consultar" data-id-ticket="${row.id_ticket}" data-toggle="tooltip" title="" class="btn btn-icon-only fs-5 p-1 me-1 btn bg-gradient-danger" data-bs-original-title="Consultar Ticket" aria-label="Consultar Ticket"><i class="fa-solid fa-clock-rotate-left"></i></span>
                            </div>`;
				}
			},
    ...
    TicketsCancelados.getTicketsCancelados();	
    portal.getOptionsCombo("#prioridades", "prioridad");
    portal.getOptionsCombo("#portales", "portales");

});

var TicketsCancelados = {
	
	tablaTicketsCancelados: false,


    
	getTicketsCancelados: function(){

		let url = portal.serverUrl+'getTicketsCancelados' 
		App.ajax('GET', url, false, null).promise.done(function(response){

			if(response.success)
			{
				TicketsCancelados.tablaTicketsCancelados.clear().rows.add(response.data).draw()
            ...
            if (response.success) {
                if (response.data.length > 0) {
                    let timeline = response.data.map(item => {
                        ...
                        let icono = item.usertype === 1 ? 'ni ni-single-02' : 'ni ni-bell-55';
    ...
    llenaTabla: function(){
        ...
        if (prioridades.length === 0 && portales.length === 0) {
            ...
            let url = portal.serverUrl+'TicketsFiltradosCancelados' 
            App.ajax('POST', url, false, obj).promise.done(function(response){
